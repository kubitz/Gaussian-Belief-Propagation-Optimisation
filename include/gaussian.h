...
public:
    ...
    Gaussian marginalize(uint32_t i, uint32_t j) const {
        ...
        if (i == 0) {
            Eigen::VectorXd eta_a = eta_(Eigen::seq(0, 1));
            Eigen::VectorXd eta_b = eta_(Eigen::seq(2, 3));

            Eigen::MatrixXd lam_bb = lam_(Eigen::seq(2, 3), Eigen::seq(2, 3));
            Eigen::MatrixXd lam_ab = lam_(Eigen::seq(0, 1), Eigen::seq(2, 3));
            Eigen::MatrixXd lam_bb_inv = lam_bb.inverse();

            eta = eta_a - lam_ab * lam_bb_inv * eta_b;
            
            Eigen::MatrixXd lam_ba = lam_(Eigen::seq(2, 3), Eigen::seq(0, 1));
            Eigen::MatrixXd lam_aa = lam_(Eigen::seq(0, 1), Eigen::seq(0, 1));

            lam = lam_aa - lam_ab * lam_bb_inv * lam_ba;
        }

        if (i == 2) {
            Eigen::VectorXd eta_a = eta_(Eigen::seq(2, 3));
            Eigen::VectorXd eta_b = eta_(Eigen::seq(0, 1));

            Eigen::MatrixXd lam_bb = lam_(Eigen::seq(0, 1), Eigen::seq(0, 1));
            Eigen::MatrixXd lam_ab = lam_(Eigen::seq(2, 3), Eigen::seq(0, 1));
            Eigen::MatrixXd lam_bb_inv = lam_bb.inverse();

            eta = eta_a - lam_ab * lam_bb_inv * eta_b;
            
            Eigen::MatrixXd lam_ba = lam_(Eigen::seq(0, 1), Eigen::seq(2, 3));
            Eigen::MatrixXd lam_aa = lam_(Eigen::seq(2, 3), Eigen::seq(2, 3));
